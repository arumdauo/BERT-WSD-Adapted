...
from utils.aat_dataset import get_example_sentences, get_glosses, get_all_lemma_names
...
def main():
    ...
    parser.add_argument(
        "--use_augmentation",
        action='store_true',
        help="Whether to augment training dataset with example sentences"
    )
    parser.add_argument(
        "--AAT_dataset_path",
        type=str,
        required=True,
        help="Path to AAT dataset consisting of a .xlsx file."
    ...
    AAT_dataset_path = Path(args.AAT_dataset_path)
    ...
    with open(csv_path, 'w', encoding='utf-8', newline='') as f:
        ...
        def _write_to_csv(_id, _sentence, _lemma, _pos, _gold_keys):
            ...
            if lemma not in dict_lemma_sense_info.keys():
                sense_info = get_glosses(_lemma.lower(), AAT_dataset_path)
                dict_lemma_sense_info[lemma] = sense_info
            else:
                sense_info = dict_lemma_sense_info[lemma]
            print(_lemma, sense_info)
            ...
            targets = [sense_keys.index(k.lower()) for k in _gold_keys if "getty" in k]
        ...
        with open(txt_path, 'r', encoding='utf-8') as g:
            for doc in tqdm(xml_root):
                for sent in doc:
                    ...
                    for token in sent: #tokens are 'wf' or 'instance'
                    ...
                    for id_, start, end, lemma, pos in instances:
                        ...
                        gold_keys = gold[1:]
                        if id_ != gold[0]:
                            print(id_, "id diverso")
                            print(gold[0], "gold[0] diverso")
        ...
        if args.use_augmentation:
            print("Creating additional training data using example sentences...")
            counter = 0
            for lemma_name in get_all_lemma_names():
                print(f"Processing {pos}...")
                for lemma in tqdm(list(lemma_name)):
